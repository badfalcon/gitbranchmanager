...
- You can also launch it from the SCM (Source Control) view title bar, or from the **Git Souji** icon in the Activity Bar.
...
2. Run **Git Souji: Open** from the Command Palette
   ...
   - Or click the **Git Souji** icon in the Activity Bar
...
### Commands
- `gitsouji.openCleaner` — **Git Souji: Open** (opens the branch manager webview)
...
- SCMビュー（Source Control）のタイトルバー、またはアクティビティバーの **Git Souji** アイコンからも起動できます
...
#### 削除キュー（アクティビティバーのビュー）
- Git Souji 専用の **アクティビティバーコンテナ** に **Deletion Queue** ツリービューを表示
...
- キューが空のときは Welcome ビューから **Open Git Souji** を起動可能
