...
dry-run では、確定後のリリースセクションが `npm test` の前に表示される。Marketplace の Changelog タブと GitHub Release の本文にそのまま載るテキストなので、内容が違っていればテストの完了を待たずに中断できる。
...
6. Open VSX へ publish（publisher の namespace が未作成なら先に作成する）
7. GitHub Release を作成し、`.vsix` を添付、`CHANGELOG.md` の該当セクションを本文に転記
8. 2 つの publish の結果を集計し、どちらかが失敗していればジョブを失敗させる

5 と 6 は互いに独立しており、片方が失敗しても、もう片方と 7 は実行される。片方のレジストリだけ公開されて GitHub Release が作られない、という中途半端な状態を避けるためである。ただし 8 で必ずジョブが赤くなるので、失敗が見逃されることはない。どちらが失敗したかは 8 のログに出る。
...
publisher 名の namespace は Open VSX 側に事前登録が必要だが、ワークフローが publish の前に `ovsx create-namespace` を実行するので手動で作る必要はない。すでに存在する場合はエラーになるが、その失敗は無視して publish に進む。
...
### 片方のレジストリへの publish だけ失敗した

ジョブは赤くなるが、GitHub Release は作成済みで、もう片方のレジストリへの公開も完了している。復旧が必要なのは失敗したレジストリだけである。

ワークフロー全体の再実行はできない（成功した側が「同一バージョンは公開済み」で失敗するため）。ただし失敗した実行の Artifacts に、実際に publish された `.vsix` がそのまま残っているので、それをダウンロードして失敗した側に publish する。ローカルで再ビルドすると、公開済みのものと異なるバイナリを片方にだけ配ってしまう恐れがあるため、artifact の再利用を優先する。
